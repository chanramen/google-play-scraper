...
function buildUrl (opts: DeveloperRequestOptions): string {
  const { lang, devId, country } = opts;
  const url = `${BASE_URL}/store/apps`;
  const path = isNaN(parseInt(opts.devId))
    ? `/developer`
    : `/dev`;
...
function developer (opts: DeveloperRequestOptions): Promise<AppItemFullDetail[]> {
  return new Promise(function (resolve, reject) {
    ...
    const options : FetchOptions = Object.assign<FetchOptions, RequestOptions | undefined>({
      url: buildUrl(opts),
      method: Method.GET,
      followRedirect: true
    }, opts.requestOptions);
